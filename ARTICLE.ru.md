...
Облачные модели (GPT-4, Claude) решают это грубой силой — контекст 128K токенов, просто вставляй всё. Но если вы работаете с **локальной** моделью на своей видеокарте — контекст ограничен. Qwen2.5-Coder-7B видит ~8000 токенов за раз. Это 300-400 строк кода. Один файл — и места на ответ уже не остаётся.
...
**Одна модель на всё.** Навигация, агенты, кодер, ревьюер — на одной Qwen2.5-Coder-7B. Качество diff ограничено возможностями 8B-модели.
...
## Почему эта архитектура похожа на мозг

При разработке Cognit не было сознательного намерения воспроизвести модели нейронауки. Архитектура выросла из практического вопроса: как заставить маленькую модель эффективно работать с большой кодовой базой. Но ретроспективно результат почти точно ложится на **предиктивное кодирование** — теорию о том, как сам мозг обрабатывает информацию.

**KV-cache как предиктивная модель.** Мозг не хранит сырые сенсорные данные — он поддерживает сжатое представление мира, позволяющее предсказывать будущие входные сигналы. Паттерн Cognit — то же самое: не копия файлов, а модель, которая «понимает» код и может отвечать на вопросы о нём.

**Обнаружение устаревания как ошибка предсказания.** В предиктивном кодировании мозг замечает, когда предсказания расходятся с входящими сигналами. Post-commit хук сравнивает хеши файлов с сохранёнными представлениями — буквально детектор рассогласования между «убеждениями» модели и реальностью.

**Ленивое переобучение как байесовское обновление.** Мозг не пересчитывает все убеждения при каждом новом стимуле. Он обновляет только ставшие релевантными. Cognit работает так же — хук помечает паттерны как устаревшие, но переобучение происходит только при следующем `/ask`, когда это знание действительно понадобится.

**Пайплайн как иерархический вывод.** Каждый агент (навигатор → аналитик → архитектура → стиль → кодер → ревьюер) — слой, который сжимает и уточняет представление сверху вниз — подобно слоям коры, от сенсорного входа к абстрактным рассуждениям и обратно.

**Агенты как специализированные зоны коры.** Каждый агент имеет собственную «модель мира» (KV-cache), обученную на своей области. Они не знают всего — они вносят свою экспертизу в общий процесс вывода.

Карл Фристон назвал бы это **активным выводом** (active inference) — система, минимизирующая разрыв между моделью и реальностью через два механизма: обновление модели (retrain) или изменение мира (patch). Cognit делает и то, и другое.

Это не было спроектировано сверху вниз из теории. Это построено снизу вверх из интуиции о том, как должно работать персистентное понимание. Тот факт, что результат сходится с байесовской теорией мозга, предполагает, что дистилляция контекста — нечто большее, чем инженерный приём. Возможно, это отражает что-то фундаментальное в том, как должны работать системы знаний.

---

## Где это находится сейчас

Работающий прототип. Полный цикл: задача → навигация → пайплайн агентов → diff → применение — работает на реальных проектах. Одна модель (Qwen2.5-Coder-7B, ~5 GB VRAM), всё in-process, без внешних зависимостей кроме llama-cpp-python и tree-sitter.
...
*Реализовано на Python с использованием Qwen2.5-Coder-7B (трансформер, GGUF-квантизация), tree-sitter для навигации по коду и BM25 для поиска символов. Все операции выполняются локально.*
