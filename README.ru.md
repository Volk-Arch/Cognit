...
### BM25 + LLM Reranking — поиск по символам

По извлечённым символам строится BM25-индекс — классический алгоритм текстового поиска. Запрос пользователя разбивается на токены и ранжируется по релевантности символов.

Поверх BM25 работает **LLM-ранжирование**: модель семантически оценивает топ-20 кандидатов и выбирает 8 наиболее релевантных. Это помогает в случаях, когда BM25 промахивается — например, функция `process_data` найдена по задаче «исправить обработку данных».

```
Запрос: «bayes update»
  BM25 top-20 → LLM rerank → top-8:
  • main.py: bayes_update() [строки 42-67]     ← наиболее релевантен
  • main.py: validate_input() [строки 12-25]
```

Индекс **инкрементальный** — при повторном запуске пропускает файлы с неизменённым хешем. Кеш хранится в `_code_index.json`. Ранжирование можно отключить через `"rerank": false` в `.echo.json`.
...
**Ограниченный семантический поиск.** Навигация по коду работает через BM25 + LLM-ранжирование. BM25 — лексический, совпадение ключевых слов. LLM-ранжирование добавляет семантическое понимание, но стоит ~2-5с на запрос. Если оба промахиваются, в задаче нужно указывать конкретные имена функций/классов.
...
Cognit/                  ← этот репозиторий
  ...
  cognit_index.py        ← tree-sitter навигатор (AST + BM25 + LLM rerank)
