...
Локальный AI-ассистент для кодовых баз. Одна модель (Qwen2.5-Coder-7B), всё in-process, без API-ключей.

```mermaid
flowchart TD
    task([задача]) --> S1

    subgraph Search["Каскадный поиск"]
        S1["① BM25 символы"] --> S2["② LLM расширение"]
        S2 --> S3["③ grep по коду"]
        S3 --> S4["④ поиск зависимостей"]
        S4 --> S5["⑤ LLM ранжирование"]
        S5 --> S6["⑥ полный скан"]
    end

    S6 --> P1

    subgraph Pipeline["Пайплайн агентов"]
        P1[навигатор] --> P2[аналитик]
        P2 --> P3[контекст / арх / стиль]
        P3 --> P4[кодер]
        P4 --> P5[ревьюер]
    end
    ...
    subgraph Hook["Post-Commit хук (без модели)"]
        H1[git diff] --> H2[пометить stale + обновить BM25]
    end

    Hook -.->|stale флаг| Cache

    subgraph Cache["KV-Cache по веткам"]
        C1["save_pattern"] <-->|"stale → retrain"| C2["ask_pattern"]
    end

    subgraph Branches["Политики веток"]
        B1["main — пересоздание при коммите"]
        B2["feature/* — накопление диалога"]
    end
...
Главная проблема маленьких моделей — ограниченный контекст. Qwen2.5-Coder-7B видит ~8000 токенов за раз. Если скормить ей файл + все соглашения по стилю + архитектуру + задачу — контекст переполнится, и на ответ не останется места.
...
В feature-ветках каждый вопрос дописывается в KV-cache — модель помнит всю историю разговора. **Лёгкий post-commit хук** (`cognit_hook.py`) определяет изменённые файлы без загрузки модели — помечает затронутые паттерны как устаревшие и инкрементально обновляет BM25-индекс. Устаревшие паттерны переобучаются лениво при следующем `/ask`.
...
### Каскадный поиск — от BM25 до полного скана

Поиск нужных файлов — каскадный процесс, каждый шаг запускается только если предыдущие не нашли достаточно:

| Шаг | Стоимость | Когда |
|---|---|---|
| **[1] BM25** по символам | бесплатно, <100мс | всегда |
| **[2] LLM расширение запроса** | ~2-3с | BM25 нашёл < 5 |
| **[3] grep** по кодовой базе | бесплатно, <500мс | всё ещё < 5 |
| **[4] поиск зависимостей** (AST) | бесплатно, <200мс | обогащение результатов |
| **[5] LLM ранжирование** | ~2-3с | > 3 кандидатов |
| **[6] полный скан** по частям | ~30-120с | ничего не найдено (крайний случай) |

```
Запрос: «исправить обработку данных»
  [1] BM25           → 0 результатов (нет лексического совпадения)
  [2] LLM расширение → предлагает "process_data, DataHandler, parse_input"
      BM25(расширен.) → 8 результатов
  [4] зависимости    → +3 вызывающих process_data()
  [5] LLM ранжиров.  → топ-8 по релевантности
```

Индекс **инкрементальный** — при повторном запуске пропускает файлы с неизменённым хешем. Кеш хранится в `_code_index.json`. LLM-шаги можно отключить через `"rerank": false` в `.echo.json`.

### ChatML — формат общения с моделью

Qwen2.5-Coder использует формат ChatML для разделения ролей. llama-cpp-python не применяет шаблон автоматически, поэтому Cognit оборачивает каждый запрос вручную:
...
**Поиск эвристический, не идеальный.** Каскадный поиск (BM25 → LLM расширение → grep → зависимости → ранжирование → полный скан) покрывает большинство случаев, но очень абстрактные запросы без лексического пересечения могут потребовать указания конкретных имён функций/классов.
...
**Одна модель на всё.** Навигация, агенты, кодер, ревьюер — всё работает на одной Qwen2.5-Coder-7B. Это удобно (один процесс, ~5 GB VRAM), но качество diff ограничено возможностями 8B-модели.
...
Cognit/                  ← этот репозиторий
  ...
  cognit_transformer.py  ← Transformer-бэкенд (KV-cache, Qwen)
  cognit_index.py        ← tree-sitter навигатор (AST + BM25 + grep + deps)
  cognit_pipeline.py     ← конфигурация пайплайна
  cognit_core.py         ← утилиты (git, паттерны, хэши, stale detection)
  cognit_hook.py         ← лёгкий post-commit хук (без загрузки модели)
  cognit_patch.py        ← применение unified diff
  cognit_agents.py       ← работа с agents/
  cognit_i18n.py         ← билингвальные сообщения (EN/RU)
